...
class ChessBoard(Mobject):
    def __init__(self, fen, includeboard, **kwargs):
    ...
    def get_chess_pieces(self, type):
        matrix = np.zeros((self.dims[0], self.dims[1]), dtype=np.int8)
        for i in range(self.dims[0]):
            for j in range(self.dims[1]):
                if self.fen_rows[i][j] == type:
                    matrix[i][j] = 1
        return matrix
    
    def remove_piece(self, i, j):
        piece_at = self.icons[i][j]
        return FadeOut(piece_at)
    
    def pieces_to_bits(self):
        for i in range(self.dims[0]):
            for j in range(self.dims[1]):
                piece_at = self.icons[i][j]
                if piece_at is not None:
                    self.remove(piece_at)
                bit_to_add = Text(str(int(piece_at is not None))).move_to(RIGHT * i + LEFT * 3.5 + UP * j + DOWN * 3.5)
                self.add(bit_to_add)
